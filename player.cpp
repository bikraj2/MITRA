...
//! [2]
    ...
    music *controls = new music();
    controls->setState(m_player->playbackState());
    controls->setVolume(m_audioOutput->volume());
    controls->setMuted(controls->isMuted());

    connect(controls, &music::play, m_player, &QMediaPlayer::play);
    connect(controls, &music::pause, m_player, &QMediaPlayer::pause);
    connect(controls, &music::stop, m_player, &QMediaPlayer::stop);
    connect(controls, ::next, m_playlist, &QMediaPlaylist::next);
